...
|---------|---------|--------------------------------------|------------------------------------------------------------------------------|
| ATT-001 | Done | AttestedBlock Struct and Constructor | `tests/attestation/test_attested_block_constructor.rs`: field wiring, empty `SignerBitmap`, `Pending`, `aggregate_signature` == proposer `Signature`. |
| ATT-002 | Done | AttestedBlock Methods                | `tests/attestation/test_attested_block_methods.rs`: 0/50/100% paths, bitmap delegation, soft finality below/at/above 67%, `hash` == `block.hash()`. |
