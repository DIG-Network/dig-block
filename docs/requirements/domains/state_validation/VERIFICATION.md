...
| STV-005 | implemented | Height/Time Lock Evaluation | Integration: `tests/test_stv_005_height_time_locks.rs` — 15 tests covering all 8 `AssertionKind` variants (pass + fail each), missing-coin-for-relative yields `CoinNotFound`, multi-assertion first-failure halts. Relative seconds uses `AVG_BLOCK_SECONDS=10` estimate pending a future `CoinLookup` extension with per-coin timestamps. Chia Check 21 parity. |
| STV-006 | implemented | Proposer Signature Verification | Integration: `tests/test_stv_006_proposer_signature.rs` — 5 tests: valid signature passes; tampered signature rejects; wrong pubkey rejects; tampered header invalidates prior signature; zero signature rejects. Uses `chia_bls::verify` inside `validate_state`. Shared helpers `stv_test_proposer_keypair` / `stv_sign_proposer` in `tests/common.rs` reused by STV-002..005 happy paths. |
