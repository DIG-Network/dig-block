...
| SVL-006 | implemented | Block Merkle Root and Integrity Checks     | `tests/test_svl_006_merkle_root_integrity.rs`: each SVL-006 test-plan row (`InvalidSpendsRoot`, `DuplicateOutput`, `DoubleSpendInBlock`, roots, `InvalidFilterHash`, slash caps, `InvalidSlashProposalsRoot`, `TooLarge`, plus `Ok`); `tests/common.rs` `sync_block_header_for_validate_structure` aligns headers for `Ok` paths. |
