...
| HSH-008 | implemented | Receipts Root Computation | `tests/test_hsh_008_receipts_root.rs`: empty → `EMPTY_ROOT`; leaf = SHA-256(bincode(`Receipt`)); explicit `MerkleTree` parity with `compute_receipts_root`; single/multi determinism; order sensitivity; `ReceiptList::from_receipts` / `finalize` roots match `compute_receipts_root`. |
