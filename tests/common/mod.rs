...
use dig_block::{BlockSigner, CoinLookup, L2Block, L2BlockHeader};
...
///
/// Note: Returns a placeholder struct until BLK-001 adds real fields.
/// Will be updated to populate all header fields once L2BlockHeader is defined.
pub fn test_header() -> L2BlockHeader {
    // Placeholder — will be replaced in BLK-001/BLK-002 when L2BlockHeader has fields.
    test_header_at_height(1)
}

/// Create a test L2BlockHeader at a specific block height.
///
/// Note: Returns a placeholder struct until BLK-001 adds real fields.
pub fn test_header_at_height(_height: u64) -> L2BlockHeader {
    // Placeholder — the height parameter will be used once L2BlockHeader has a height field.
    // For now, return the default stub.
    let _ = _height;
    L2BlockHeader::stub()
}
